                ...
                <uni-list-item :show-arrow="false" :right-text="item.createTime | filterTime"
                    ...
                    <template v-slot:default>
                        <view class="amount-text">
                            <text class="amount-sign">{{item.inOut | filterType(that)}}</text><text class="amount-yuan">￥</text>{{item.value | filterPrice}}</view>
...
import {filterBalanceLog} from "../../../api/finance";

export default {
    name: "balance-log-list",
    ...
    filters: {
        ...
        filterType(inOut, that) {
            const [obj] = that.inOutList.filter(v => v.value === inOut)
    ...
    methods: {
        ...
        getBalanceLogList(){
            ...
            filterBalanceLog(param).then(res=>{
                if(res.retCode===0){
